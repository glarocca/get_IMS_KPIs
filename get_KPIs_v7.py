...
from jirautils import getCustomersComplains, getSLAViolations

__author__    = "Giuseppe LA ROCCA"
__email__     = "giuseppe.larocca@egi.eu"
__version__   = "$Revision: 0.9"
__date__      = "$Date: 16/04/2024 06:55:17"
...
def checkTokens(env):
    ...
    try:
        ...
        print("[%s] CONFLUENCE_SERVER_URL is *DOWN* or not responding! " %colourise("red", "ERROR"))

    else:
        print("[%s] CONFLUENCE_SERVER_URL is *UP* " %colourise("green", "INFO"))
    ...
    try:
        ...
        print("[%s] JIRA_SERVER_URL is *DOWN* or not responding! " %colourise("red", "ERROR"))
    else:  
        print("[%s] JIRA_SERVER_URL is *UP* " %colourise("green", "INFO"))
...
def getKPIs(env, totalPages, total_CPU, total_RAM, total_HEPSPEC, total_Block_Storage, total_Object_Storage):
           ...
           for customer in customers['detailLines']:
               ...
               if ("FitSM" not in customer['title']) and \
                  ("Users" not in customer['title']):
                   ...
                   if (env['PARENT'] == "76679055"):
                      # (IMS) Inactive CustomersDB
                      item=parsingHTML(details[11], 'a').split()
                      _month=(parsingHTML(details[11], 'a').split()[1])
                      _year=(parsingHTML(details[11], 'a').split()[-1])
                   ...
                   if (int(_year) <= int(env['DATE_TO'][0:4]) and _month in semester):
                      ...
                      if (env['PARENT'] == "1867983"):
                         ...
                         if re.search(r'\bFINALIZED\b', details[44]):
                         ...
                         customer = {
                            ...
                            "Customer SLA Contact": parsingHTML(details[6], 'div'),
                            "Customer Status": customer_status,
                            "SLA Status": parsingHTML(details[44], 'div'),
                            "Resources": resources
                         }
                      ...
                      if (env['PARENT'] == "76679055"):
                         # (IMS) Inactive CustomersDB
                         KPI_SLM_SLA_1b = KPI_SLM_SLA_1b + 1

                         customer = {
                            "Customer Name": customer_name,
                            "Customer Start date": parsingHTML(details[11], 'a'),
                            "Customer SLA Contact": parsingHTML(details[8], 'div'),
                         ...
                         reporting['INACTIVE'] = int(reporting['INACTIVE']) + 1
                         update = 1
    ...
    complains = getCustomersComplains(env, complains)

    if (env['LOG'] == "DEBUG"):
        if complains:
           print("\n[%s] Customers *COMPLAINS* in the reporting period" %colourise("red", "WARNING"))
           if env['DATE_TO'][5:7] == "06":
              print(colourise("green", "[INFO]"), \
              "Reporting Period = %s.01-%s " %(env['DATE_TO'][0:4], env['DATE_TO'][5:7]))
           else:
              print(colourise("green", "[INFO]"), \
              "Reporting Period = %s.07-12 " %env['DATE_TO'][0:4])

           #print(json.dumps(complains, indent=4, sort_keys=False))
           console = Console()
           ...
           print("\n[%s] No Customers *COMPLAINS* in the reporting period." %colourise("green", "INFO")) 

    for complain in complains:
        if (complain['Status'] == "TODO") or (complain['Status'] == "NEW"):
            KPI_CRM_Complains_1 = KPI_CRM_Complains_1 + 1
        if (complain['Status'] == "IN PROGRESS") or (complain['Status'] == "ON HOLD"):
    ...
    if (env['LOG'] == "DEBUG"):
        if violations:
           print("\n[%s] VO *SLA VIOLATIONS* in the reporting period " %colourise("red", "WARNING"))
           if env['DATE_TO'][5:7] == "06":
              print(colourise("green", "[INFO]"), \
              "Reporting Period = %s.01-%s " %(env['DATE_TO'][0:4], env['DATE_TO'][5:7]))
           else:
...
def main():
    ...
    print("Configuring environment settings in progress...")
    print("This operation may take some time to complete. Please wait!\n")
    ...
    print("[%s] Check the status of the servers and tokens " %colourise("green", "INFO"))
    ...
    if (confluence_token > 0 and jira_token > 0):
       ...
       #if env['DATE_TO'][5:7] == "06":
       #   print(colourise("green", "[INFO]"), \
       #           "Reporting Period = %s.01-0%s " %(env['DATE_TO'][0:4], env['DATE_TO'][5:7]))
       #else:   
       #   print(colourise("green", "[INFO]"), \
       #           "Reporting Period = %s.07-12 " %env['DATE_TO'][0:4])
       ...
       if update:
          if (env['PARENT'] == "1867983"):
              print(colourise("cyan", "[SPACE] (IMS) CustomersDB"))
          
          #else:   
          #      print(colourise("cyan", "[SPACE]"), \
          #           "%s [%s]" %(env['PARENT'],"(IMS) Inactive Customers"))
          ...
          if (env['PARENT'] == "1867983"):
            ...
            table.add_row(
              "KPI:CRM.Customers.1", 
              "Number of Customer DB entries that are active in any form (cumulative)", 
            ...
            table.add_row(
              "KPI:SLM.SLA.2",
              "Number of SLA violations (relative)",
              str(KPIs['KPI:SLM.SLA.2'])
            )

            table.add_row(
              "KPI:CRM.Complains.1",
              "Number of NEW Customer Complains (e.g.: status = TODO, NEW) (relative)",
              str(KPIs['KPI:CRM.Complains.1'])
            )
            
            table.add_row(
              "KPI:CRM.Complains.2",
              "Number of OPEN Complains (e.g. status = IN PROGRESS, ON HOLD) (relative)",
          ...
          #   print("- Number of retired VO SLAs (total, cumulative)")
          #   print("* KPI:SLM.SLA.1b = %d" %KPIs['KPI:SLM.SLA.1b'])
